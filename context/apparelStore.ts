...
import { APERTURES_LIBRARY, CAMERA_ANGLES_LIBRARY, COLOR_GRADING_PRESETS, EXPRESSIONS, FABRIC_TYPES_LIBRARY, FOCAL_LENGTHS_LIBRARY, LIGHTING_PRESETS, SHOT_TYPES_LIBRARY, LIGHTING_DIRECTIONS_LIBRARY, LIGHT_QUALITIES_LIBRARY, CATCHLIGHT_STYLES_LIBRARY } from '../constants';
...
export const createApparelSlice: StudioStoreSlice<ApparelSlice> = (set, get) => ({
  ...
  saveModel: async (imageB64) => {
    ...
    try {
        const modelDetails = await geminiService.describeModel(imageB64);
  ...
  generateAIModel: async (prompt) => {
    ...
    try {
      ...
      const imageB64 = await geminiService.generateWithImagen(finalPrompt, '3:4');
      if (imageB64) {
  ...
  addApparelItem: async (base64) => {
      ...
      const artDirectorPromise = geminiService.getArtDirectorSuggestions(base64);
      const analysisPromise = geminiService.analyzeApparel(base64);
  ...
  suggestAndApplyLayering: async () => {
    ...
    try {
      ...
      const orderedIds = await geminiService.suggestLayering(itemsToLayer);
  ...
  analyzeAndSetModelLighting: async () => {
      ...
      try {
          const description = await geminiService.describeImageStyle(uploadedModelImage);
  ...
  applyArtDirectorSuggestion: (suggestion) => {
    ...
    const suggestedBackground = get().BACKGROUNDS_LIBRARY.find(b => b.id === suggestion.backgroundId);
