-- Migration: User Provisioning Trigger
-- Purpose: Automatically create public.users row when auth.users row is created
-- This ensures public.users.id = auth.users.id immediately, so RLS self-select policies work
