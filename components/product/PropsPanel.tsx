...
export const PropsPanel: React.FC = () => {
    const { productControls, updateProductControl, fetchPropSuggestions, isSuggestingProps, propSuggestions, error, stagedAssets } = useStudio();
    
    const handleAddProp = (prop: string) => {
        const currentProps = productControls.customProps.trim();
        const newProps = currentProps ? `${currentProps}, ${prop}` : prop;
        ...
        <div className="h-full flex flex-col space-y-6 overflow-y-auto pr-1">
            ...
            <div className="p-3 rounded-lg bg-zinc-800/50 border border-white/10 space-y-3">
                 <h3 className="font-semibold text-zinc-100 flex items-center gap-2">
                    <Wand2 size={18} /> AI Prop Assistant
                </h3>
                 <button 
                    onClick={fetchPropSuggestions} 
                    disabled={isSuggestingProps}
                ...
                >
                    {isSuggestingProps ? <Loader2 size={16} className="animate-spin" /> : <Sparkles size={16} />}
                    {isSuggestingProps ? 'Analyzing product...' : 'Suggest Props'}
                </button>
                 {error && <p className="text-xs text-red-400 text-center">{error}</p>}

                 {propSuggestions.length > 0 && (
                     <div className="flex flex-wrap gap-2 pt-2">
                        {propSuggestions.map((prop, index) => (
                            <button
                                key={index}
                                onClick={() => handleAddProp(prop)}
                                className="text-xs bg-zinc-800 text-zinc-200 px-2.5 py-1.5 rounded-full hover:bg-zinc-700 transition-colors border border-white/10 shadow-inner-soft"
                            >
                                + {prop}
                            </button>
                        ))}
