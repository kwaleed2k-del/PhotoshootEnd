...
export const StudioView: React.FC = () => {
  ...
  const showEmptyState = !isGenerating && !generatedVideoUrl && (!generatedImages || generatedImages.length === 0) && !previewImage && !showDesignPreview;
  const showPreview = !isGenerating && !generatedVideoUrl && (!generatedImages || generatedImages.length === 0) && previewImage && !showDesignPreview;
  ...
  return (
    <div className="relative w-full min-h-full flex flex-col gap-3">
        <div className="relative min-h-[400px] bg-zinc-925/50 backdrop-blur-sm rounded-lg flex items-center justify-center p-2 sm:p-4 border border-white/10 shadow-2xl shadow-black/50 overflow-hidden">
             {(() => {
                ...
                if (generatedVideoUrl && activeImageIndex === null) {
                    return (
                        <video
        ...
        {((imagesToDisplay && imagesToDisplay.length > 0) || generatedVideoUrl) && !error && (
            <div className="flex-shrink-0 min-h-[120px] bg-zinc-900/60 backdrop-blur-xl rounded-lg border border-white/10 p-3 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="w-full sm:flex-1 flex flex-nowrap items-center gap-3 h-full overflow-x-auto model-filter-scrollbar min-w-0">
                    {imagesToDisplay && imagesToDisplay.map((imageB64, index) => (
                        imageB64 ? (
                            <div 
                                key={index} 
                                className={`relative group h-24 sm:h-full aspect-[3/4] rounded-md cursor-pointer transition-all duration-200 overflow-hidden flex-shrink-0 ${index === activeImageIndex ? 'ring-2 ring-offset-2 ring-violet-500 ring-offset-zinc-925 shadow-lg shadow-violet-700/30' : 'hover:opacity-80'}`}
                            ...
                            >
                             ...
                             <div 
                                ...
                                className="relative h-24 sm:h-full aspect-[3/4] rounded-md overflow-hidden flex-shrink-0 bg-zinc-800 shimmer-bg"
                            />
                        )
                    ))}
                    {generatedVideoUrl && videoSourceImage && (
                        <div 
                            className={`relative group h-24 sm:h-full aspect-[3/4] rounded-md cursor-pointer transition-all duration-200 overflow-hidden flex-shrink-0 ${activeImageIndex === null ? 'ring-2 ring-offset-2 ring-violet-500 ring-offset-zinc-925 shadow-lg shadow-violet-700/30' : 'hover:opacity-80'}`}
                            onClick={() => setActiveImageIndex(null)}
                        >
                            <img src={videoSourceImage} alt="Video thumbnail" className="object-cover w-full h-full" />
                             <div className="absolute inset-0 bg-black/50 flex items-center justify-center transition-opacity opacity-70 group-hover:opacity-100">
                ...
                {((activeImage || (generatedVideoUrl && activeImageIndex === null)) && !isGenerating && !isEditing) && (
