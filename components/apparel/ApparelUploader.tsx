...
import { Shirt, Sparkles, Loader2, Eye } from 'lucide-react';
...
export const ApparelUploader: React.FC = () => {
    ...
    const [detectingApparel, setDetectingApparel] = useState<boolean>(false);
        ...
        for (const file of acceptedFiles) {
            ...
            reader.onload = async (event: ProgressEvent<FileReader>) => {
                if (event.target?.result) {
                    ...
                    // Add the item first
                    addApparelItem(imageBase64);
                    
                    // Then detect apparel type
                    setDetectingApparel(true);
                    try {
                        const detection = await apparelDetectionService.detectApparelType(imageBase64);
                        
                        // Update the most recently added item with detected information
                        const currentApparel = apparel;
                        const lastIndex = currentApparel.length;
                        
                        // Update category and description based on AI detection
                        if (detection.category) {
                            updateApparelItemCategory(lastIndex, detection.category);
                        }
                        if (detection.description) {
                            updateApparelItemDescription(lastIndex, detection.description);
                        }
                        
                        console.log("✅ Apparel type detected:", detection);
                    } catch (error) {
                        console.error("Failed to detect apparel type:", error);
                    } finally {
                        setDetectingApparel(false);
                    }
                }
            };
            reader.readAsDataURL(file);
        }
    }, [addApparelItem, apparel, updateApparelItemCategory, updateApparelItemDescription]);
