...
const DEFAULT_DRAFT: StudioJobDraft = {
	template_id: STUDIOS_TEMPLATES.apparel[0].id,
	images_count: 2,
	variations: 1,
	brand: '',
	palette: '',
...
export default function ApparelStudioPage() {
	...
	const [draft, setDraft] = useState<StudioJobDraft>(() => loadDraft('apparel') ?? DEFAULT_DRAFT);
	...
	const templates = STUDIOS_TEMPLATES.apparel;
	...
	const handleSaveJob = async () => {
		...
		try {
			const result = await createStudioJob('apparel', {
	...
	const renderedStep = useMemo(() => {
		switch (step) {
				...
				return (
					<form className="space-y-3">
						...
						<div className="flex gap-2 text-sm">
							<button
								type="button"
								onClick={() => saveDraft('apparel', draft)}
							...
							<button
								type="button"
								onClick={() => {
									const stored = loadDraft('apparel');
			...
			case 2:
				return (
					<div className="space-y-4">
						<StudioSummaryCard kind="apparel" draft={draft} />
						<StudioEstimator kind="apparel" draft={draft} balance={balance ?? undefined} />
						<div className="flex flex-wrap gap-3">
							...
							<button onClick={handleReset} className="rounded border border-white/20 px-4 py-2 text-sm text-white">
								Reset
							</button>
						</div>
						{submitState.message ? (
	...
	}, [balance, draft, handleSaveJob, submitState.status, submitState.message, templates]);

	return (
		<div className="space-y-6">
			<header className="space-y-1">
				<h1 className="text-3xl font-semibold text-white">Apparel Studio</h1>
				<p className="text-sm text-zinc-400">Preview the on-model experience and estimate credits before going live.</p>
			</header>
			<StudioStepper steps={steps} active={step} onPrev={() => setStep((prev) => Math.max(prev - 1, 0))} onNext={() => setStep((prev) => Math.min(prev + 1, steps.length - 1))}>
				{renderedStep}
			</StudioStepper>
			<section className="space-y-3 rounded border border-white/10 bg-white/5 p-4">
				...
				{Array.isArray(jobs) ? (
					jobs.length ? (
						...
						<p className="text-sm text-zinc-400">No jobs yet. Once the endpoint is live, history will appear here.</p>
					)
